...
% ────────────────────────────────────────────────────────────────────────────
\begin{frame}[shrink=4]{Front-End Technology}
...
% ────────────────────────────────────────────────────────────────────────────
\begin{frame}[shrink=4]{Front-End --- Why These Choices?}\small

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{block}{Why Vanilla JS?}
\begin{itemize}\itemsep=0pt
    \item \textbf{Zero build step} --- no bundlers
    \item \textbf{No dependencies} --- instant load
    \item \textbf{Simpler debugging} --- no abstractions
    \item \textbf{Direct deployment} --- static files
    \item Modern ES6+: async/await, modules
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{itemize}\itemsep=0pt
    \item \textbf{Utility-first} --- rapid UI dev
    \item \textbf{Responsive} built-in (sm/md/lg)
    \item \textbf{Dark mode} --- class-based toggle
...
% ────────────────────────────────────────────────────────────────────────────
\begin{frame}[shrink=4]{Back-End --- Why These Choices?}\small

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{block}{Why Node.js?}
\begin{itemize}\itemsep=0pt
    \item \textbf{JS everywhere} --- same language
    \item \textbf{Non-blocking I/O} --- concurrent
    \item \textbf{npm ecosystem} --- largest registry
    \item \textbf{Vercel-native} --- serverless ready
...
\begin{itemize}\itemsep=0pt
    ...
    \item \textbf{Industry standard} --- huge community
    \item \textbf{Middleware pipeline} --- composable
    \item \textbf{Easy REST APIs} --- simple routes
    \item v5: async error handling
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\textbf{Helmet} (headers) + \textbf{CORS} + \textbf{rate-limit} + \textbf{compression}
\end{block}
\end{frame}
...
\small
\begin{itemize}\itemsep=0pt
    \item Widely documented
    \item ACID compliant
    \item InnoDB: FK support
...
\textbf{Flexibility} --- MySQL for local dev, PostgreSQL for cloud. No lock-in.
\end{exampleblock}
\end{frame}
...
\begin{tikzpicture}[scale=0.85, transform shape,
    box/.style={draw, rounded corners=4pt, minimum width=2.4cm, minimum height=0.85cm, align=center, font=\small\bfseries},
...
\node[box, fill=primary!15, draw=primary] (api) at (4.2, 0) {Express.js\\API};
\node[box, fill=orange!15, draw=orange!70] (dal) at (8.4, 0) {Data Access\\Layer};

% Database nodes
\node[box, fill=blue!10, draw=blue!50] (mysql) at (12.2, 1) {MySQL 8};
\node[box, fill=blue!10, draw=blue!50] (pg) at (12.2, -0.2) {PostgreSQL};
\node[box, fill=gray!10, draw=gray!50] (mem) at (12.2, -1.4) {In-Memory};

% External services
\node[box, fill=purple!10, draw=purple!50] (fx) at (4.2, -2.2) {FX API};
\node[box, fill=green!10, draw=green!50] (vercel) at (4.2, 2) {Vercel};
...
\begin{tikzpicture}[scale=0.82, transform shape,
    table/.style={draw, rounded corners=3pt, minimum width=2.4cm, align=left, font=\tiny, inner sep=3pt},
...
% ════════════════════════════════════════════════════════════════════════════
% Split Key Features into two slides to avoid overflow
\begin{frame}{Key Features (1/2)}
...
\begin{itemize}\itemsep=0pt
    \item \textbf{80--100}: Excellent
    \item \textbf{60--79}: Good
    \item \textbf{40--59}: Needs improvement
    \item \textbf{0--39}: At risk
\end{itemize}
\end{block}
\end{frame}
